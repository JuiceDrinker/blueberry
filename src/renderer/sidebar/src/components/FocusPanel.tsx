...
import { CheckCircle2, Circle, XCircle, MessageSquare, Target, Shield, ShieldOff, Loader2, ExternalLink, X } from 'lucide-react'
...
const TaskItem: React.FC<{
    ...
    onActOnNextStep: (tabId: string, nextStep: string) => void
}> = ({ task, focusedTaskTitle, onFocus, onUnfocus, onSwitchToTab, onComplete, onActOnNextStep }) => {
        ...
        <div className={cn(
            ...
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {task.nextStep && (
                        <button
                            onClick={() => task.tabIds[0] && onActOnNextStep(task.tabIds[0], task.nextStep!)}
                            className={cn(
                    ...
                    {task.status === 'in_progress' && (
                        <div className="flex items-center gap-2 mt-3">
                            {isFocused ? (
                                <button
                                    onClick={() => { onComplete(task.id); onUnfocus(); }}
                                    className={cn(
            ...
            <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                    <span className="text-xs text-amber-600 dark:text-amber-400 font-medium">
                        {distractions.length} distraction {distractions.length === 1 ? 'tab' : 'tabs'}
                    </span>
...
export const FocusPanel: React.FC<FocusPanelProps> = ({ taskList, isLoading, isUpdating, onBackToChat, onActOnNextStep }) => {
    ...
    const handleCompleteTask = (taskId: string) => {
        setLocalTasks((prev) =>
            prev.map((t) => t.id === taskId ? { ...t, status: 'completed' as const, nextStep: null } : t)
        )
        if (focusedTaskTitle) {
